...
VOICE_NAMES=("clone:announcer_e0" "clone:turret_e0" "clone:announcer_e0")
...
print(json.dumps({
    ...
    "instruct": os.environ["BASE_TTS_INSTRUCT"],
...
dispatch_request() {
  ...
  request_json="$(build_tts_request_json "$voice_name" "$text" "$voice_index")"
  : >"$buffer_file"
  BUFFER_FILES+=("$buffer_file")
  DONE_FILES+=("$done_file")
  STATUS_FILES+=("$status_file")

  echo "Dispatching line $((index + 1)) with voice=${voice_name} for queued playback"
  (
    status=0
    if curl -fsS \
