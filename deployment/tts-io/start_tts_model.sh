...
MODEL_NAME="${TTS_MODEL_NAME:-Qwen/Qwen3-TTS-12Hz-0.6B-Base}"
SERVER_PORT="8091"
CUDA_DEVICE="${CUDA_VISIBLE_DEVICES:-0}"
STAGE_CONFIG_PATH="${TTS_STAGE_CONFIG_PATH:-$SCRIPT_DIR/qwen3_tts_small_gpu.yaml}"
...
if [ -f "$ENV_FILE" ]; then
  ...
  source "$ENV_FILE"
  set +a
  MODEL_NAME="${TTS_MODEL_NAME:-$MODEL_NAME}"
  STAGE_CONFIG_PATH="${TTS_STAGE_CONFIG_PATH:-$STAGE_CONFIG_PATH}"
fi
...
if [ ! -f "$STAGE_CONFIG_PATH" ]; then
  echo "Missing TTS stage config: $STAGE_CONFIG_PATH"
  exit 1
fi
